...
_VARIANTS = [
    ("1.2", "float", "f", "%f"),
    ("3.14", "double", "", "%f"),
    ("2.718281828", "long double", "L", "%Lf"),
    ("0.0", "float", "f", "%f"),
    ("-5.5", "double", "", "%f"),
    ("1e10", "long double", "L", "%Lf"),
    ("100.0", "float", "f", "%f"),
    ("0.123456789", "double", "", "%f"),
    ("-0.0001", "long double", "L", "%Lf"),
    ("42.0", "float", "f", "%f"),
]


class Module_1_Submodule_3_task_6(BaseTaskClass):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        seed_value = 0 if self.seed is None else self.seed
        variant = _VARIANTS[seed_value % len(_VARIANTS)]
        self.value_str = variant[0]
        self.type_name = variant[1]
        self.suffix = variant[2]
        self.format_spec = variant[3]

    def generate_task(self) -> str:
        return (
            f"Напишите программу, которая выводит значение {self.value_str}, сохраненное в переменной типа `{self.type_name}`. "
            f"Литерал `{self.value_str}` должен быть записан с правильным суффиксом, чтобы его тип в точности соответствовал типу переменной без неявного преобразования. "
            f"Вывод осуществите с помощью `printf` и спецификатора: "
            f"`%f` для `float` и `double`, "
            f"`%Lf` для `long double`.\n"
    ...
    def _check_includes_and_macros(self) -> Optional[str]:
        ...
        if self.type_name == "float":
            if not re.search(r'\b[0-9]+(\.[0-9]*)?([eE][+-]?[0-9]+)?[fF]\b', clean_code):
                return "Ошибка"
        elif self.type_name == "double":
            if re.search(r'\b[0-9]+(\.[0-9]*)?([eE][+-]?[0-9]+)?[fFlL]\b', clean_code):
                return "Ошибка"
            if not re.search(r'\b[0-9]+\.[0-9]*([eE][+-]?[0-9]+)?\b', clean_code):
                return "Ошибка"
        elif self.type_name == "long double":
            if not re.search(r'\b[0-9]+(\.[0-9]*)?([eE][+-]?[0-9]+)?[lL]\b', clean_code):
                return "Ошибка"

        if self.type_name == "long double":
            if not re.search(r'%Lf', clean_code):
                return "Ошибка"
        else:
            if not re.search(r'%f', clean_code):
                return "Ошибка"
    ...
    def _build_reference_program(self) -> str:
        return (
            ...
            "\n"
            "int main() {\n"
            f"    {self.type_name} x = {self.value_str}{self.suffix};\n"
            f'    printf("{self.format_spec}", x);\n'
            "    return 0;\n"
            "}\n"
        )
