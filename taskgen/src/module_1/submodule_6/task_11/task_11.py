...
class Module_1_Submodule_6_task_11(BaseTaskClass):
    ...
    def generate_task(self) -> str:
        return (
            f"Напишите программу, которая вычисляет наибольший общий делитель (НОД) двух натуральных чисел {self.A} и {self.B} "
            f"по алгоритму Евклида с вычитанием. Требования:\n"
            f"- В программе обязательно использовать ключевое слово `goto` для организации повторяющихся действий.\n"
            f"- Запрещается применять циклы `for`, `while`, `do-while`.\n"
            f"- Вывести полученный НОД (только число, можно с переводом строки `\\n`).\n"
        )
    ...
    def _check_includes_and_macros(self) -> Optional[str]:
        ...
        if not re.search(r'\bmain\s*\(', clean_code):
            return "Ошибка"
        if not re.search(r'\bgoto\b', clean_code):
            return "Ошибка"
        if re.search(r'\b(for|while|do)\b', clean_code):
            return "Ошибка"
        return None
