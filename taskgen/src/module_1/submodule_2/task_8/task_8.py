...
_VARIANTS: dict[int, VariantSpec] = {
    0: VariantSpec("char", "%d", "CHAR_MIN", "CHAR_MAX"),
    1: VariantSpec("short", "%d", "SHRT_MIN", "SHRT_MAX"),
    2: VariantSpec("int", "%d", "INT_MIN", "INT_MAX"),
    3: VariantSpec("long", "%ld", "LONG_MIN", "LONG_MAX"),
    4: VariantSpec("long long", "%lld", "LLONG_MIN", "LLONG_MAX"),
}


class Module_1_Submodule_2_task_8(BaseTaskClass):
    ...
    def generate_task(self) -> str:
        v = self.variant
        return (
            f"  Напишите программу, которая выводит через пробел минимальное и максимальное значение типа `{v.type_name}`,\n"
            f"  используя макросы `{v.min_macro}` и `{v.max_macro}` из `<limits.h>`.\n"
            "  Программа должна содержать `#include <stdio.h>`, `#include <limits.h>` и функцию `main`.\n"
    ...
    def _check_includes_and_macros(self) -> Optional[str]:
        ...
        if not re.search(r'#include\s*<\s*limits\.h\s*>', clean_code):
            return "Ошибка: программа содержит не все необходимые модули."
        if not re.search(r'\b' + re.escape(v.min_macro) + r'\b', clean_code):
            return f"Ошибка: программа содержит не все необходимые модули."
