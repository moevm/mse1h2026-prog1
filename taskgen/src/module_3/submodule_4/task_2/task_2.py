...
class Module3_Submodule4_Task2(BaseTaskClass):
    ...
    def _get_test_driver_code(self) -> str:
        names = ["my_strcpy", "custom_strcpy", "str_copy", "manual_strcpy"]
...
"""
    ...
    def _generate_tests(self):
        ...
        base_strings = [
            "hello", "C_Programming", "test123", "a", 
            "string_copy", "buffer", "data", "x", "abc_def"
        ]
        while len(base_strings) < self.tests_num:
            base_strings.append("".join(
                random.choices("abcdefghijklmnopqrstuvwxyz0123456789_", k=random.randint(3, 15))
            ))
    ...
    def check_sol_prereq(self) -> Optional[str]:
        ...
        if re.search(r'\bstrcpy\b', self.solution):
            return "Ошибка: запрещено использовать библиотечную функцию strcpy."
