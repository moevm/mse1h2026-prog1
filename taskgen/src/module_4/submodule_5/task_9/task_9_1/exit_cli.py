...
def create_task_exit(args) -> ExitTask:
    return ExitTask(**get_common_cli_args(args))


def add_cli_args_exit(parser: argparse.ArgumentParser):
    parser.add_argument("-s", "--seed", type=int, default=0)  # seed не используется, но оставим для единообразия
    ...
    parser.set_defaults(func=create_task_exit)


ExitCLIParser = CLIParser(
    name="exit",
    add_cli_args=add_cli_args_exit,
)