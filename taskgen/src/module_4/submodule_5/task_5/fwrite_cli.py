...
from .fwrite_task import FWriteTask


def create_task_fwrite(args) -> FWriteTask:
    return FWriteTask(**get_common_cli_args(args))


def add_cli_args_fwrite(parser: argparse.ArgumentParser):
    ...
    parser.set_defaults(func=create_task_fwrite)


FWriteCLIParser = CLIParser(
    name="fwrite",
    add_cli_args=add_cli_args_fwrite,
)