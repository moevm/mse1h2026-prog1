...
def create_task_errno(args) -> ErrnoTask:
    return ErrnoTask(**get_common_cli_args(args))


def add_cli_args_errno(parser: argparse.ArgumentParser):
    ...
    parser.set_defaults(func=create_task_errno)


ErrnoCLIParser = CLIParser(
    name="errno",
    add_cli_args=add_cli_args_errno,
)